...
namespace Arch.Web.Controllers
{
    public class WorkController : AdminController
    {
        ...
        public ActionResult Unit() { return View(); }
        ...
        public ActionResult InsertTask(TaskParamDto model)
        {
            ...
            try
            {
                ...
                // kaydetme işlemi başlatmak için yarım kalan iş oldugunda kaydettirmeyeceğimiz için bu şekilde başlatıyoruz. 
                ...
                if (bool.Parse(Resources.Parameter.IsSendMailForWork))
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (assigned.Email != null)
                            {
                                var sr = new StreamReader(Server.MapPath(@"/Views/Shared/EmailTemplate/") + "TaskDetailEmail.html"); string body = sr.ReadToEnd();
                                ...
                                {
                                    var hata = new ExceptionLog
                                    {
                                        StackTrace = ex.InnerException == null ? ex.StackTrace : ex.InnerException.StackTrace,
                                        Message = ex.InnerException == null ? ex.Message : ex.InnerException.Message,
                                        ExceptionUrl = HttpContext.Request.RawUrl.ToString(),
                                    ...
                                    _workService.InsertExceptionLog(hata);


                                    using (FileStream stream = new FileStream(Server.MapPath("~/log/ExLog.txt"),
                                                           FileMode.Append, FileAccess.Write))
                                    {
                                        StreamWriter writer = new StreamWriter(stream);
                                        StringBuilder strbuilder = new StringBuilder();
                                        strbuilder.Append("tarih:" + "Yeni İş açarken mail gönderiminde" + "n");
                                        strbuilder.Append("hata nerede :" + DateTime.Now.ToString() + "n");
                                        strbuilder.Append("Mesaj :" + ex.Message + "n");
                                        strbuilder.Append("Hedef Yer:" + ex.StackTrace + "n");
                                        strbuilder.Append("........................................");
        ...
        private void Write(string filePath, string fileName, string strLog, bool isAppend = true)
        {
            try
            {
        ...
        //serdar
        [HttpPost]
        public ActionResult UpdateUnit(string Name, int Id)
        {
            if (string.IsNullOrEmpty(Name) == false && Id != 0)
            {
                var entity = _workService.FindUnit(Id);
                entity.Name = Name;
                entity.Code = entity.Code;
                entity.UnitTypeId = entity.UnitTypeId;
                entity.ParentId = entity.ParentId;
                entity.PdksUnitId = entity.PdksUnitId;
                entity.YbsUnitId = entity.YbsUnitId;
                entity.IsActive = true;
                _workService.UpdateUnit(entity);
                _uow.SaveChanges();
                return AjaxSuccess();
            }
            return null;
        ...
        public ActionResult Register(int? id)
        {
            ...
            if (id == null)
            {
                var t = new RegisterParamDto();
                return View(t);
            }
            var result = _workService.GetPersonById(id != null ? id.Value : 0);
            if (result == null)
                Response.Redirect("/Work/RegisterList");
            return View(id != null ? result : new RegisterParamDto());
        }
        ...
        public ActionResult ParameterDetail(int? id)
        {
            Session.Abandon();
            if (id == null)
            {
                var t = new Parameters();
                return View(t);
            }
            var result = _workService.GetParameterById(id != null ? id.Value : 0);
            if (result == null)
                Response.Redirect("/Work/Parameters");
            return View(id != null ? result : new Parameters());
        }
        ...
        [HttpPost]
        public ActionResult UpdateParameter(ParameterListDto model)
        {
            if (string.IsNullOrEmpty(model.Name) == false && model.Id != 0)
            {
                var entity = _workService.FindParameter(model.Id);
                entity.Name = model.Name;
                // entity.Code = entity.Code;
                entity.Description = model.Description;
                entity.Value = model.Id == 1 || model.Id == 2 ? EnDeCode.Encrypt(model.Value, StaticParams.SifrelemeParametresi) : model.Value;
                //entity.IsActive = true;
                _workService.UpdateParameter(entity);
                _uow.SaveChanges();
                return AjaxSuccess();
            }
            return null;
        ...
        [HttpPost]
        public ActionResult InsertPerson(RegisterParamDto model)
        {
            try
            {
                var asd = _workService.GetUnits(model.UnitId);
                // kaydetme işlemi başlatmak için yarım kalan iş oldugunda kaydettirmeyeceğimiz için bu şekilde başlatıyoruz. 
                // _uow.BeginTransaction();
                var entity = new Person
                {
                    ...
                    Password = EnDeCode.Encrypt(model.Password, StaticParams.SifrelemeParametresi),
                    Initials = model.Name.Substring(0, 1).ToUpper() + model.Surname.Substring(0, 1).ToUpper(),
                    Email = model.Email,
                    Phone = model.Phone,
                    IsActive = true,
                    //ImageId = 189
                ...
                _workService.InsertPerson(entity);
                _uow.SaveChanges();
                ...
                if (bool.Parse(Resources.Parameter.IsSendMailForWork))
                {
                    try
                    {
                        if (entity.Email != null)
                        {
                            ...
                            MailHelper.SendMail("Yeni Üye - " + result[0].Title, messageBody, entity.Email);
                        }
                    }
                    catch (Exception ex) { }
                }

                return AjaxMessage("", "İşlem başarıyla gerçekleşti.$$window.location='/Work/Register?Id=" + entity.Id + "'", MessageTypes.funcAndMessage);
            }
            catch (Exception ex)
            {
                _uow.Rollback();
        ...
        [HttpPost]
        public ActionResult UpdatePerson(RegisterParamDto model)
        {
            var entity = _personService.FindPerson(model.Id);
            ...
            try
            {
                ...
                entity.Password = EnDeCode.Encrypt(model.Password, StaticParams.SifrelemeParametresi);
                entity.Initials = model.Name.Substring(0, 1).ToUpper() + model.Surname.Substring(0, 1).ToUpper();
                entity.Email = model.Email;
                entity.Phone = model.Phone;
                entity.IsActive = true;
                //ImageId = 189
                entity.PersonId = Accesses.PersonId;
                entity.CreatedDate = DateTime.Now;

                _workService.UpdatePerson(entity);
                _uow.SaveChanges();
                ...
                if (bool.Parse(Resources.Parameter.IsSendMailForWork))
                {
                    try
                    {
                        if (entity.Email != null)
                        {
                            ...
                            MailHelper.SendMail("Şifre Degişti - " + result[0].Title, messageBody, entity.Email);
                        }
                    }
                    catch (Exception ex) { }
                }
        ...
        {
            return Json(_workService.GetUnits(), JsonRequestBehavior.AllowGet);
        }

        public ActionResult GetUnitsList()
        {
            return Json(_workService.GetUnitsList(), JsonRequestBehavior.AllowGet);
        }

        public ActionResult GetPersonsUnits()
        {
            return Json(new
            {
                Persons = _personService.GetPersons(),
        ...
        {
            return Json(_workService.GetPersonsList(), JsonRequestBehavior.AllowGet);
        }


        public ActionResult GetParameters()
        {
            return Json(_workService.GetParameters(), JsonRequestBehavior.AllowGet);
        }


        [HttpPost]
        public ActionResult InsertUnit(string Name)
        {
            try
            {
                if (string.IsNullOrEmpty(Name) == false)
                {
                    int sayi = _workService.GetLastUnitId() + 1;
                    var entity = new Unit
                    {
                        ParentId = 2633,
                        UnitTypeId = 2092,
                    ...
                    _workService.InsertUnit(entity);
                    _uow.SaveChanges();
