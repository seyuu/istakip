...
$.zsytem.LeftSideBar = {
    ...
    setMenuHeight: function () {
        if (typeof $.fn.slimScroll != 'undefined' && $.zsytem.Browser.IsMobileDevice() == false) {
            var configs = $.zsytem.options.leftSideBar;
...
$.zsytem.DropdownMenu = {
    ...
    dropdownEffect: function (target) {
        var effectIn = $.zsytem.options.dropdownMenu.effectIn, effectOut = $.zsytem.options.dropdownMenu.effectOut;
...
$.zsytem.ToTitleCase = function (txt) {
    if (txt != null) {
...
$.zsytem.ToEmpty = function (text) {
    return text == null ? "" : text;
}
$.zsytem.Loading;
$.zsytem.WaitMeIsShow = true;
$.zsytem.Initialize = function () {
    $.zsytem.Loading = $('[data-loading="cardloading"]').parents('.card').waitMe({
        effect: "pulse",
        text: '',
...
$.zsytem.CreateChart = function (chartId, title, field, series, data, maxAxis) {
    $("#" + chartId).kendoChart({
        dataSource: data,
        title: {
            text: title
            ...
            error: function (xhr, errorType, exception) {
                ...
                xhr.statusText == "func" || xhr.statusText == "funcAndMessage") {
                    if (xhr.statusText != "success")
                        isReload = false;
                    $.zsytem.Notify.Show(rTextSplit[1], rTextSplit[0], xhr.statusText);
                }
            }
        });
    },
        ...
        if (model != undefined) {
            ...
            $.ajax({
                ...
                success: function (e) {
                    ...
                    $.zsytem.Notify.Show(rTextSplit[1], rTextSplit[0], "info");
                },
                error: function (xhr, errorType, exception) {
                    $.zsytem.Loading.waitMe('hide');
                    var rText = xhr.responseText;
...
function formSendModel(e) {
    ...
    $.zsytem.Server.Post(model, url, eval(func));
    $("[data-btn]").removeAttr("disabled");
}
function GetValue(e) {
...
// Activate
$(function () {
    $.zsytem.Browser.activate();
    $.zsytem.LeftSideBar.activate();
    $.zsytem.Navbar.activate();
    $.zsytem.DropdownMenu.activate();
    $.zsytem.Input.activate();
    $.zsytem.Select.activate();
