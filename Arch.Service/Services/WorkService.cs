...
namespace Arch.Service.Services
{
    public class WorkService : IWorkService
    {
        ...
        private readonly IGenericRepository<Parameters> _parameterRepository;
        ...
        public WorkService(IUnitofWork uow)
        {
            ...
            _parameterRepository = uow.GetRepository<Parameters>();
        ...
        {
            return _parameterRepository.Find(id);
        }
        ...
        public void UpdateParameter(Parameters entity)
        {
            _parameterRepository.Update(entity);
        }
        
        public List<RegisterParamDto> GetPersonsList()
        {
            var result = _objectRawSql.Execute(StaticParams.DefaultConnstr, typeof(RegisterParamDto),
                //sql functions içerisinde 
                @"select p.Id,p.UserName,p.Initials,p.Name,p.Surname,p.Password,p.Email,p.IsActive,p.Phone,p.PersonId,p.CreatedDate,u.Name as 'UnitName',u.Code from common.person as p
                inner join common.unit as u 
                on p.UnitId=u.Id").Cast<RegisterParamDto>().ToList();

            var resultCount = _objectRawSql.Execute(StaticParams.DefaultConnstr, typeof(int),
                "select count(*) from common.person").Cast<int>().SingleOrDefault();
        ...
        {
            var result = _objectRawSql.Execute(StaticParams.DefaultConnstr, typeof(Parameters),
            //sql functions içerisinde 
            @"select * from common.Parameters").Cast<Parameters>().ToList();
            //var resultCount = _objectRawSql.Execute(StaticParams.DefaultConnstr, typeof(int),
            //    "select count(*) from common.parameters").Cast<int>().SingleOrDefault();
            //if (result.Count > 0) result[0].ResultCount = resultCount;
            return result;

            //var result = (from a in _parameterRepository.GetAll()
            //              orderby a.Name ascending
            //              select new
            //              {
            //                  Code = a.Code,
        ...
        public RegisterParamDto GetPersonById(int id)
        {
            var person = _personRepository.Find(id);
            if (person == null)
                return null;

            var unit = _unitRepository.GetAll().Where(p => p.Id == person.UnitId).FirstOrDefault();
            var result = new RegisterParamDto();
            result.Code = unit.Code;
            result.CreatedDate = person.CreatedDate;
            result.Email = person.Email;
            result.Id = person.Id;
            //result.ImageId = person.ImageId;
            result.Initials = person.Initials;
            result.IsActive = person.IsActive;
            result.Name = person.Name;
            result.Password = EnDeCode.Decrypt(person.Password, StaticParams.SifrelemeParametresi);
            result.PersonId = person.PersonId;
            result.Phone = person.Phone;
            result.Surname = person.Surname;
            result.Unit = unit;
            result.UnitId = person.UnitId;
        ...
        {
            var task = _parameterRepository.Find(id);
            if (task == null)
                return null;
            ...
            result.Value = task.Value.Length>20? EnDeCode.Decrypt(task.Value, StaticParams.SifrelemeParametresi) : task.Value;
            result.IsActive = true;
            return result;


        }
        public object GetUnitsList()
        {
            var result = _unitRepository.GetAll().OrderBy(x => x.Name).ToList();
            return result;
        }

        public int GetLastUnitId()
        {
            return _unitRepository.GetAll().OrderByDescending(x => x.Id).Select(p => p.Id).First();
        }

        public void InsertExceptionLog(ExceptionLog entity)
        {
            _exceptionLogRepository.Insert(entity);
        }
